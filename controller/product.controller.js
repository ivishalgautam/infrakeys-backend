...
async function updateProductById(req, res) {
  ...
  const updatedSlug = slug.trim().toLowerCase().split(" ").join("-");
  ...
  try {
    const { rows, rowCount } = await pool.query(
      `UPDATE products SET title = $1, about = $2, sub_category_id = $3, meta_title = $4, meta_desc = $5, keywords = $6, slug = $7 WHERE id = $8 returning *`,
      [
        title,
        about,
        sub_category_id,
